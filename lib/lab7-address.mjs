...
function defaultAddress(group, router, slot) {
  ...
  if (router === 1 && slot === "lan") return { port: "GigabitEthernet0/0", ip: `${IP_BASE}.${lan1}.1`, prefix: "24", note: `PC1 ใช้ ${IP_BASE}.${lan1}.10/24 · Gateway ${IP_BASE}.${lan1}.1` };
  ...
  if (router === 2 && slot === "lan") return { port: "GigabitEthernet0/0", ip: `${IP_BASE}.${lan2}.1`, prefix: "24", note: `PC2 ใช้ ${IP_BASE}.${lan2}.10/24 · Gateway ${IP_BASE}.${lan2}.1` };
...
export function createBoard() {
  ...
  const add = (group, router, slot, label, segment, peer, extension = false) => rows.push({
    ...
    ...defaultAddress(group, router, slot), revision: 0,
  });
  ...
  return { version: 1, revision: 0, updatedAt: null, rows };
}
